...
import SmsRetrieverPlugin, { ReceivedMessage } from '../../plugins/SmsRetrieverPlugin';
...
const LoginWithPhone: React.FC = () => {
  ...
  useEffect(() => {
    const setupSmsRetriever = async () => {
      try {
        if (isPlatform('android')) {
          await SmsRetrieverPlugin.startSmsReceiver();
          SmsRetrieverPlugin.addListener('onSmsReceive', (receivedMessage: ReceivedMessage) => {
            if (receivedMessage.message) {
              const otp = receivedMessage.message.match(/\d{4}/);
              //set otp in otp field
              if (otp) {
                setValue('otp', otp[0]);
              }
            }
    ...
    const cleanupSmsRetriever = async () => {
      try {
        if (isPlatform('android')) {
          await SmsRetrieverPlugin.removeSmsReceiver();
        }
      } catch (error) {
        console.log(error);
      }
  ...
  const onSendOtp = async () => {
    ...
    try {
      if(isPlatform('android')){
        const smsPlugin = await SmsRetrieverPlugin.getAppSignature();
        await axiosPublic.post(api_routes.login_phone_otp_auto_read, { phone: getValues().phone, hash: smsPlugin.signature });
      }else{
