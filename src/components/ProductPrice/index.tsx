...
const ProductPrice: React.FC<{
    ...
    if(cart_product_item().length>0){
        return <p className="product-card-price">
            {cart_product_item()[0].product_price.discount !== 0 && <del>&#8377;{cart_product_item()[0].product_price.price}</del>}<span><b>&#8377;{cart_product_item()[0].product_price.discount_in_price}</b> <small> / {cart_quantity_specification}</small></span>
        </p>
    }
    if(product_prices.length > 0){
        ...
        return <p className="product-card-price">{price[0].discount !== 0 && <del>&#8377;{price[0].price}</del>}<span><b>&#8377;{price[0].discount_in_price}</b> <small> / {cart_quantity_specification}</small></span></p>
    }
